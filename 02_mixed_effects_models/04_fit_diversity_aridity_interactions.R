# =============================================================================
# 04_fit_diversity_aridity_interactions.R
#
# Purpose : Fit the functional-diversity x aridity interaction models: the
#           baseline model plus ONE FD metric and its interaction with
#           aridity_index,
#               GPP ~ LAI + Temperature + Precipitation + aridity_index
#                     + aridity_index * FD + (1 | ecoprovince)
#           with a Gaussian spatial correlation structure (corGaus, with nugget).
# Inputs  : ../data/analysis_ready_tables/data.csv
# Outputs : LME_GPP_LAI_T_P_AI_FD_AI_ecoprovince_SA_model_list.rds (one lme per FD metric)
# Run     : Rscript 04_fit_diversity_aridity_interactions.R   (working directory = this folder)
#           Run order: after 03_fit_composition_aridity_interactions.R
...
# Fit one interaction model per FD metric in parallel
model_list <- foreach(FD = FD_list, .packages = c("nlme", "dplyr")) %dopar% {
  print(FD)
  formula <- as.formula(paste("GPP ~ LAI + Temperature + Precipitation + aridity_index + aridity_index * ", FD))
...
saveRDS(model_list, "LME_GPP_LAI_T_P_AI_FD_AI_ecoprovince_SA_model_list.rds")
