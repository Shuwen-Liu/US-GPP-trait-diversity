# =============================================================================
# 06_export_full_model.R
#
# Purpose : Export the forward-selected full model (05_forward_selection_full_model.R):
#           fixed-effect estimates with 95% CIs and p-values, and model
#           performance (marginal / conditional R2, AIC, ICC, RMSE) together
#           with the partial R2 and delta AIC relative to the baseline model.
# Inputs  : nlme_forward_selection_PC3_final_model.rds   (from 05)
#           LME_GPP_LAI_T_P_AI_ecoprovince_SA.rds         (baseline, from 01)
# Outputs : nlme_forward_selection_PC3_final_model_estimate.csv
#           nlme_forward_selection_PC3_final_model_performance.csv
# Run     : Rscript 06_export_full_model.R   (working directory = this folder)
#           Run order: after 05_forward_selection_full_model.R
# Feeds   : Fig. 3 ("Full model" bar; read by 11_plot_fig3.py)
# =============================================================================
...
# Load the full model and the baseline model
model <- readRDS("nlme_forward_selection_PC3_final_model.rds")
...
# Export CSV
write.csv(results_df, "nlme_forward_selection_PC3_final_model_estimate.csv", row.names = FALSE)
...
# Export CSV
write.csv(df_t, "nlme_forward_selection_PC3_final_model_performance.csv", row.names = FALSE)
